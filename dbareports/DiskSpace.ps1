...
BEGIN
{
	...
	try
	{
        ...
        $datatable.Columns[5].DataType = [decimal]
        $datatable.Columns[6].DataType = [decimal]
        $datatable.Columns[7].DataType = [int]
	}
	catch
	{
	...
	foreach ($server in $sqlservers)
	{
		...
		foreach ($disk in $disks)
		{
			...
			if (!$diskname.StartsWith("\\"))
			{
                ...
                [decimal]$total = [math]::Round(($disk.Capacity/1gb),2)
				[decimal]$free = [math]::Round(($disk.FreeSpace/1gb),2)
				[int]$percentfree = [math]::Round((($disk.FreeSpace / $disk.Capacity) * 100),0)
