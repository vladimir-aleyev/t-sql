...
80 – 90 per second is normal, anything more we need to check the lazy writer/sec and checkpoint counters, if these counters are also relatively high then, it is a memory constraint.
...
-- calculate page writes per second
SELECT
	(@value2 - @value1) / datediff(ss,@time1,@time2) [Page writes/sec];